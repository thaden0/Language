# editor-dom — Moby DOM flagship example

The target-feel program from `designs/moby/dom/techdesign-00-dom-overview.md` §2,
...
|---|---|---|
| `MobyApp app = MobyApp();` | `class MobyApp : App` — owns the Document, `start()` alias | D01 |
...
| `FileDialog openDialog = FileDialog("open")` | `Moby::Dialogs::FileDialog` over `sysListDir` | D07 |
...
| `app.start()` | alias for `run()` on `MobyApp` | D01 |
...
2. **`uses Moby::Dialogs`** instead of `uses MyApp::Dialogs` — the framework ships it.
...
MOBY_SCRIPT=1 trident run
```

## Scripted session (MOBY_SCRIPT=1)

1. Initial DOM queries — verifies the tree is correctly built
2. `outerMarkup` of the toolbar — round-trip snapshot
3. Tab to focus the textarea, type "Hello, Moby!" — verifies the input path
