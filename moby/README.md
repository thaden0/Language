# Moby — Leviathan TUI Framework

Moby is a retained-mode terminal UI framework for the Leviathan programming language.
...
// Tier 2 — comptime import() (external .moby asset file):
IComponent ui2 = FlexContainer(import("main.moby"));
...
Use `moby/tests/harness/dom_helpers.lev` in your test's `trident.toml`:

```lev
uses Moby;
uses Moby::Dom;

// dom_helpers.lev adds to MobyTest namespace:
MobyTest::click(app, "#ok-button");
MobyTest::type(app, "#search", "hello");
MobyTest::chord(app, "^s");                   // DOM hotkey grammar
MobyTest::expectText(app, "#status", "saved", "status after save");

string mouse = MobyTest::encodeMouse(10, 5, 0, true);  // SGR press at (10,5)
```
