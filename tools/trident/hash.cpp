...
Sha256::Sha256() {
    std::memcpy(state_, kH0, sizeof(state_));
}
...
void Sha256::final(unsigned char out[32]) {
    ...
    // Append the length directly into the buffer without going through
    // update() (which would recompute bitLen_ from a "message" that isn't
    ...
    for (int i = 0; i < 8; ++i) {
        out[i * 4]     = (unsigned char)(state_[i] >> 24);
        out[i * 4 + 1] = (unsigned char)(state_[i] >> 16);
        out[i * 4 + 2] = (unsigned char)(state_[i] >> 8);
        out[i * 4 + 3] = (unsigned char)(state_[i]);
    }
}
