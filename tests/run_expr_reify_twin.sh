...
# byte-equivalence. A twin file hand-authors the exact expr::Expr::<Fn>(...)
...
if [ -z "$o" ]; then
  echo "FAIL: no expr::Expr::<Fn>(...) construction found in --expand of $orig"
  exit 1
fi
if [ -z "$t" ]; then
  echo "FAIL: no expr::Expr::<Fn>(...) construction found in --expand of $twin"
  exit 1
fi
if [ "$o" != "$t" ]; then
