...
std::string CGen::generate() {
    ...
    // known_bugs_2.md #102: name the specific uncovered sys:: native in the
    // coverage-deferral diagnostic. Object/collection/closure-wrapped natives
    // (e.g. Process -> sysSpawn, Pty -> sysPtySpawn) otherwise trip the generic
    // "does not yet cover this construct" default first — the wrapping opcode
    // fails before emission reaches the CallNativeFn that would name the native,
    // and the sink reports only that first error. A module-wide pre-scan surfaces
    // the root system-layer native up front, deterministically, regardless of
    // function/instruction emission order. Natives CGen DOES lower are listed
    // here; keep this in sync with the CallNativeFn handler in genFunction().
    static const std::unordered_set<std::string> kCoveredNatives = {
        "sysWrite", "sysReadLine", "sysRead", "byteToString", "charFromCode",
        "floatFromBits", "sysArgs", "sysThreadTransfer", "sysNow", "sysTermRaw",
        "sysTermRestore", "sysWinSize", "sysTermIsRaw", "sysSetExitCode",
        "sysExit", "log", "log2", "exp", "sin", "cos", "tan", "atan2",
    };
    // BFS from the entry over STATIC call/ctor/closure edges only (Call/NewObject/
    // MakeClosure operand b = target fn index), so the reported native is the one
    // behind the user's construct (Process's ctor -> sysSpawn) rather than an
    // unrelated native that is merely by-name-reachable elsewhere in the prelude.
    // A native found this way is genuinely uncoverable — CGen would fail emitting
    // that function anyway — so surfacing it first only sharpens the diagnostic.
    {
        std::vector<char> seen(mod_.functions.size(), 0);
        std::vector<int> work;
        if (mod_.entry >= 0 && (size_t)mod_.entry < mod_.functions.size())
            work.push_back(mod_.entry);
        std::string culprit;
        while (!work.empty() && culprit.empty()) {
            int fi = work.back(); work.pop_back();
            if (fi < 0 || (size_t)fi >= mod_.functions.size() || seen[fi] ||
                !reachable[fi]) continue;
            seen[fi] = 1;
            for (const Inst& in : mod_.functions[fi].code) {
                if (in.op == Op::CallNativeFn && !kCoveredNatives.count(in.sname)) {
                    culprit = in.sname; break;
                }
                if ((in.op == Op::Call || in.op == Op::NewObject ||
                     in.op == Op::MakeClosure) &&
                    in.b >= 0 && (size_t)in.b < mod_.functions.size())
                    work.push_back(in.b);
            }
        }
        if (!culprit.empty()) {
            sink_.error({}, "native backend: native '" + culprit + "'");
            ok_ = false;
            return "";
        }
    }
