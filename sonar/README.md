# Sonar — the standard Leviathan unit test library
...
Add Sonar as a dependency and write a two-line entry file:
...
class MathTests {
    ...
    @Test void subtracts()        { sonar::assertEqual(1, 4 - 3); }
    @Test @Skip void notYet()     { }                 // reported as "skip"
}

sonar::main();   // runs every discovered @Test, prints the report, sets the exit code
```

That is the whole surface. Mark a method `@Test`, `uses sonar;` (which you
...
- Failure is an exception: `sonar::assertEqual`/`assertTrue`/… throw an
...
- `sonar::main()` runs everything, prints the report, and sets the process
...
trigger is `uses sonar`, not the file's path. Both shapes ship as worked
examples (`sonar/examples/inline/`, `sonar/examples/sibling/`) and produce
byte-identical reports.

## Assertion vocabulary (`sonar/src/assert.lev`)
...
the message), and `fail`. See `sonar/tests/assertions/main.lev` for every
...
sonar/
  ...
  examples/
    ...
    sibling/          # app/ + tests/ as two projects ([[dep]] on app + sonar)
```

## Remaining gotchas

- **Give your test-suite entry point any name but `main`** if the file also
  `uses sonar;` — a bulk `uses` import's `main` silently wins over a
  same-named local `main`, no diagnostic (known_bugs_1.md #78). Sonar's own
  self-tests call `sonar::main()` directly (they have no local `main`);
