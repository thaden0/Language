...
void lvrt_arith(int32_t op, LvValue* out, const LvValue* l, const LvValue* r) {
    ...
    if (l->tag == LV_NONE || r->tag == LV_NONE) {
        ...
        /* Relational ops on a None operand are pinned false, never a real
         * ordering (designs/expr-reification/techdesign-01 §3.1) — mirrors
         * arithPrim (RuntimeValue.hpp) / CGen.cpp's ar(). */
        if (op >= LV_OP_LT && op <= LV_OP_GE) { out->tag = LV_BOOL; out->payload = 0; return; }
        out->tag = LV_VOID; out->payload = 0; return;
    }
