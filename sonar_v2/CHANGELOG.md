...
The v0.2.0 release adds the full Sonar DOM surface — a web-developer-friendly layer on
...
**D01 — Document, selectors, DomNode, SonarApp**
- `SonarApp : App` owns a `Document` and adds `query`/`queryAll`/`queryOrNone` sugar
...
- Three markup tiers: `dom!(\`…\`)` comptime, `import("x.sonar")` asset, runtime string
...
**D07 — Dialogs**
- `Sonar::Dialogs::FileDialog("open"|"save")`: directory listing via `std::sysListDir` +
  `std::isDir` (no O(N) probe — `request-stat-isdir.md` fulfilled); `show()` → `Promise<string|None>`
- `Sonar::Dialogs::PromptDialog`: labeled input + validator + OK/Cancel
- `Sonar::Dialogs::alert`/`confirm` re-exports (one-stop `uses Sonar::Dialogs`)

**D08 — DevTools, testing, examples, delivery** (this release)
- `Sonar::DevTools::inspector(app, hotkey="F12")`: DOM inspector overlay (tree/detail/
...
- `examples/editor-dom/`: the target-feel program, corrected and running, with scripted
  session golden test
- `sonar_v2/trident.toml`: `src/dom/*.lev` glob already covers devtools (no line added)
- `sonar_v2/README.md`: DOM section, selectors, classes, actions, bindings, inspector,
